...
def main():
    ...
    npy_list = os.listdir('/home/joann8512/NAS_189/home/LoopClassifier/npy_full_1sec')
    ...
    np.save(open(os.path.join(save_path, 'binary_full_1sec.npy'), 'wb'), short_binary)
    ...
    for i, title in enumerate(short_filenames):  # titles -> filenames -> short_filenames
        ...
        if i < len(short_filenames)*0.8:
            if short_binary[i].sum() > 0:
                tr.append(str(i)+'\t'+title)
        #elif len(short_filenames)*0.9 <= i < len(short_filenames)*0.95:
        elif len(short_filenames)*0.8 <= i < len(short_filenames)*0.9:
            if short_binary[i].sum() > 0:
                val.append(str(i)+'\t'+title)
        else:
            if short_binary[i].sum() > 0:
                test.append(str(i)+'\t'+title)
    np.save(open(os.path.join(save_path, 'train_full_1sec.npy'), 'wb'), tr) #tr_exist)
    np.save(open(os.path.join(save_path, 'valid_full_1sec.npy'), 'wb'), val) #val_exist)
    np.save(open(os.path.join(save_path, 'test_full_1sec.npy'), 'wb'), test)
