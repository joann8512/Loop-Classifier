...
class Predict(object):
    ...
    def load(self, filename):
        ...
        #print(self.model.stft.bw_Q)  # model.hconv
    ...
    def evaluate_multiclass(self, num_chunks=16):
        ...
        for line in tqdm.tqdm(filelist):
            ix, fn = line.split('\t')
            #file = open("/home/joann8512/NAS_189/home/LoopClassifier/Baseline/pred.txt","a") 
        ...
        #for i in range(len(est_array)):
        #    array = np.zeros(6)
        #    for p in range(len(est_array[i])):
        #        if est_array[i][p] > 0.5:
        #            array[p] = 1
        #        else:
        #            array[p] = 0
        #    est_array[i] = array
        #cm = confusion_matrix(gt_array, est_array)
        #fig = plt.figure()
        ##ax = fig.add_subplot(111)
        #cax = fig.matshow(cm)
        #plt.title('Confusion matrix of the classifier')
        #fig.colorbar(cax)
        #labels = ['perc', 'bass', 'chord', 'melody', 'FX', 'voice']
        #fig.set_xticklabels([''] + labels)
        #fig.set_yticklabels([''] + labels)
        #plt.xlabel('Predicted')
        #plt.ylabel('True')
        #plt.show()
        #fig.savefig('/home/joann8512/NAS_189/home/LoopClassifier/prediction/conf_matrix.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_harmonic', type=int, default=1)
    ...
    parser.add_argument('--model_save_path', type=str, default='./baseline_output')
    parser.add_argument('--model_load_path', type=str, default='./baseline_output/best_model.pth')
